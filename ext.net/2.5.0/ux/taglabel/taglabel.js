...
if(key==e.ENTER&&rawValue.length!=0){e.stopEvent();}else if(key==e.LEFT){if(pos==0&&me.inputMoving){me.tagLabel.moveInput(-1,pos);me.focus();me.selectText(0,0);}}else if(key==e.RIGHT){if(pos==rawValue.length&&me.inputMoving){me.tagLabel.moveInput(1,pos);me.focus();me.selectText(pos,pos);}}
...
return true;},checkChange:function(){if(!this.suspendCheckChange){var me=this,newVal=me.value||[],oldVal=me.lastValue;if(!me.isEqual(newVal,oldVal)&&!me.isDestroyed){me.lastValue=Ext.Array.clone(newVal);me.fireEvent('change',me,newVal,oldVal);me.onChange(newVal,oldVal);this.checkTriggerHeight();}}},checkTriggerHeight:function(){if(this.rendered){var td=this.triggerCell.elements?this.triggerCell.elements[0]:this.triggerCell;if(td&&td.getHeight()>25){this._bottomColor=td.getStyle("border-bottom-color");td.setStyle("border-bottom-width",0);if(Ext.net.ResourceMgr.theme!=="neptune"&&this.triggerEl){this.triggerEl.setStyle({"border-bottom-width":"1px","border-bottom-style":"solid","border-bottom-color":this._bottomColor});}}
else if(td&&this._bottomColor){if(Ext.net.ResourceMgr.theme!=="neptune"&&this.triggerEl){this.triggerEl.setStyle("border-bottom-width","0px");}
td.setStyle({"border-bottom-width":"1px","border-bottom-color":this._bottomColor});delete this._bottomColor;}}},initValue:function(){this.callParent(arguments);if(!this.originalValue){this.originalValue=[];}
