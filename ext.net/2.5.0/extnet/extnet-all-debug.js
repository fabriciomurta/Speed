...
Ext.form.field.ComboBox.override({
    ...
    setInitValue : function (value) {
        ...
        if (this.store.getCount() > 0) {
            ...
            this.store.on("load", set, this, { single : true });
        }
    },
...
Ext.form.field.Date.override({
    // The fix for the GitHub issue #45. Remove after Sencha fix.
    initComponent: function () {
    ...
    // End of the fix for the the GitHub issue #45.
...
Ext.form.field.Display.override({    
    fieldSubTpl: [
        '<div id="{id}" role="input" class="{fieldCls}" <tpl if="fieldStyle"> style="{fieldStyle}"</tpl>>{value}</div>',
        {
            compiled: true,
            disableFormats: true
        }
...
Ext.LoadMask.override({
    ...
    initComponent: function () {
        if (Ext.isEmpty(this.initialConfig.msg)) {
...
Ext.define('Ext.net.InfoPanelQueue', {                                                
    ...
    initQueue : function () {                
        if (!this.container) {
            this.initItems();
            Ext.EventManager.onWindowResize(this.align, this, {buffer: 200});
        }
...
Ext.define("Ext.net.Callout", {
    ...
    renderTpl: [
        '<div id="{id}-arrow" class="{baseCls}-arrow<tpl if="arrowCls"> {arrowCls}</tpl>"<tpl if="arrowStyle"> style="{arrowStyle}"</tpl>>',
            '<tpl if="Ext.isIE7m"><span class="{baseCls}-arrow-after"></span></tpl>',
        '</div>',
    ...
    onMouseMove: function (e) {
        ...
        if (t) {                    
            if (t !== (me.triggerElement && me.triggerElement.dom)) {
                me.hide();
    ...
    onDestroy: function () {
        if (this.trigger == "hover" || this.closeOnOutsideClick) {
            Ext.getDoc().un('mousedown', me.onDocMouseDown, this);
        }
    ...
    renderRow: function (record, rowIdx, out) {
        ...
        if (!isMetadataRecord) {
            ...
            if (selModel && selModel.isRowSelected) {
                if ((rowIdx + 1) < selModel.store.getTotalCount()) { // fix
            ...
            Ext.each(this.get("currentPageOnly", config) ? grid.store.getRange() : grid.store.getAllRange(), function(record, index) {
                ...
                for (i = 0, len = columns.length; i < len; i++ ) {                    
                    ...
                    if (c.dataIndex) {
                        ...
                        meta = {tdCls: "", tdAttr: "", style: ""};
                        item[Ext.String.createVarName(c.id)] = c.renderer.call(grid, null, meta, record, index, i, grid.store, grid.view); //index + 1;
                    } 
...
(function () {
    ...
    if (Ext.isGecko) {
        buf.push(".x-theme-neptune .x-gecko .x-form-text { padding: 4px 6px 2px; }"); // The GitHub issue #401. Remove after Sencha fix.

        // The GitHub issue #411. Remove after Sencha fix.
