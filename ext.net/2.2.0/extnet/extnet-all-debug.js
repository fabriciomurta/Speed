...
Ext.override(Ext.AbstractComponent, {
    ...
    destroyContentWidgets : function (checkFlag) {
        if ((this.destroyContent || !checkFlag) && !this.isNonContentable()) {
            this.destroyFromDom(this.getContentTarget().dom);
        }
    },
...
Ext.override(Ext.form.field.Text, {    
    ...
    updateButtonsWidth : function () {
        ...
        if (field.leftButtons || field.rightButtons) {
            ...
            el.setWidth(el.parent().getWidth() - w - (field.buttonsOffset || 3));
        }
    },
    ...
    getSubmitArray : function () {
        ...
        if (!this.valueModels || this.valueModels.length == 0) {
            ...
            if (!Ext.isEmpty(value)) {
                ...
                if (!record) {
                    this.store.on("load", this.syncHiddenState, this, {single:true});
                    return [{value : value}];
                }
...
// TODO: Github issue #189. Needs to remove after Sencha fix
// fix start
...
Ext.toolbar.Toolbar.override({
    onBeforeAdd: function(component) {
        ...
        } else if ((isButton || component.isFormField) && me.ui !== 'footer' && this.classicButtonStyle !== true) {
...
Ext.layout.component.field.Field.prototype.beginLayoutCycle = Ext.Function.createInterceptor(Ext.layout.component.field.Field.prototype.beginLayoutCycle, function (ownerContext) {
    ...
    return {
        ...
        show : function (config) {
            ...
            {   
                ...
                if(Ext.getBody().getHeight() >= Ext.fly(window).getHeight())
                {
                    defaultOffset[1] = -10 - Ext.getScrollbarSize().height;
                }

                if(Ext.getBody().getWidth() >= Ext.fly(window).getWidth())
                {
                    defaultOffset[0] = -10 - Ext.getScrollbarSize().width;
                }
...
Ext.grid.Panel.override({
    ...
    insertColumn : function (index, newCol, doLayout) {
        var headerCt = this.headerCt || this.normalGrid.headerCt; 
...
Ext.grid.RowEditor.override({
    onColumnAdd: function(column) {
        ...
        this.doColumnAdd(this.columns, editor && editor.id, column);
    ...
    onColumnRemove: function(column) {
        var editor = column.getEditor();
        this.doColumnRemove(this.columns, editor && editor.id, column);
...
Ext.grid.column.Column.override({
    ...
    setPadding: function(headerContext, availableHeight) {
        ...
        if (headerContext.innerCtContext || me.isComponentHeader) {
    ...
    initRenderData : function () {
        ...
        if (me.commands) {
            if (me.overOnly) {
                ...
                me.view.on("refresh", me.insertToolbars, me, {delay:10});
        ...
        if (me.grid.isTree || me.grid.ownerLockable && me.grid.ownerLockable.isTree) {
            me.grid.on("beforeitemcollapse", me.removeNodeToolbars, {column: me, root: false});
            me.grid.on("beforeitemmove", me.removeNodeToolbars, {column: me, root: true});
        }
    ...
    insertGroupToolbars : function () {
        ...
        if (this.groupCommands) {
            for (i = 0; i < groupCmd.length; i++) {
                ...
                var groupId = Ext.fly(groupCmd[i]).parent().query(".x-grid-group-hd")[0].id.substr((this.view.id + '-hd-').length),
...
Ext.define("Ext.grid.column.ComponentColumn", {
    ...
    initRenderData : function () {
        ...
        if (me.overOnly) {
            me.view.on("beforerefresh", function(){
                this.hideComponent();
            }, me);
        ...
        if (me.grid.isTree || me.grid.ownerLockable && me.grid.ownerLockable.isTree) {
            me.grid.on("beforeitemcollapse", me.removeNodeComponents, {column: me, root: false});
            me.grid.on("beforeitemmove", me.removeNodeComponents, {column: me, root: true});
        }
    ...
    insertComponents : function (start, end, row) {
        ...
        for (var i = start; i < end; i++) {
            ...
            record = this.grid.store.getAt ? this.grid.store.getAt(i) : this.view.getRecord(this.view.getNode(i));
