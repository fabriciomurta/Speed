...
this.search(this.displayField,key.toLocaleLowerCase?key.toLocaleLowerCase():key.toLowerCase(),startIndex);this.cshTask.delay(this.searchResetDelay);e.preventDefault();return false;},onRender:function(ct,position){this.callParent(arguments);this.createPicker();this.picker.on("refresh",this.calcRowsPerPage,this);},afterRender:function(){this.callParent(arguments);if(Ext.isWebKit){this.inputEl.swallowEvent("mousedown",true);}
