...
Ext.Object.each(values,function(key,value){copy[key]=me.addValues(value);});Ext.each(cmpValues,function(value){copy[value[0]]=value[1];});return copy;},insertComponent:function(first,last){this.insertComponents(first,last+1);},itemUpdated:function(record,index){this.insertComponents(index,index+1);},itemAdded:function(records,index){this.insertComponents(index,index+(records.length||1));},beforeItemClick:function(view,record,item,index,e){var cmpEl=this.cache[index]&&this.cache[index].cmp.getEl();if(cmpEl&&e.within(cmpEl)){return false;}},insertComponents:function(start,end){var me=this,nodes=me.view.all.elements,node,i,c,t,cmp,cmps,targets,trg,tpl,len,record;if(Ext.isEmpty(start)||Ext.isEmpty(end)||!Ext.isNumber(start)||!Ext.isNumber(end)){start=0;end=nodes.length;}
