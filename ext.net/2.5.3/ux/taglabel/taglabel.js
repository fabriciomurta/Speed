...
me.tagLabel.checkTab=Ext.Function.bind(this.checkTab,this);this.checkInputElChange();this.afterTagsChange();},applyEmptyText:Ext.emptyFn,afterTagsChange:function(){if(this.tagLabel){this.value=this.tagLabel.tags;}
...
rec=me.findRecordByDisplay(value);if(rec){this.addTagToInput(rec);}
...
rawValue=Ext.Array.clean(rawValue.split(this.getDelimeter()));me.inputEl.dom.value='';me.addTagToInput(rawValue);me.store.clearFilter();me.inputEl.focus();e.stopEvent();return false;}
...
do{index=index+direction;item=boundList.all.item(index);}while((index>0)&&(index<len)&&item.hasCls('x-boundlist-selected'));if(item&&item.hasCls('x-boundlist-selected')){return;}}
