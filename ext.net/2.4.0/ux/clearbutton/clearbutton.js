...
Ext.define("Ext.net.ClearButton",{extend:"Ext.util.Observable",alias:"plugin.clearbutton",hideIfEmpty:true,hideOnMouseOut:true,clearOnEsc:true,cls:"x-clear-button",constructor:function(cfg){Ext.apply(this,cfg);this.addEvents("beforeclear","clear");this.callParent(arguments);},init:function(field){this.field=field;if(field.rendered){this.initPlugin();}
else{field.on("afterrender",this.initPlugin,this,{single:true});}},initPlugin:function(){this.el=this.field.bodyEl.createChild({tag:"div",cls:this.cls});this.el.hide();this.initEvents();if(this.clearOnEsc){this.field.inputEl.on("keydown",this.onEsc,this);}
...
if(this.field.triggerWrap){right+=this.field.getTriggerWidth();}
return{right:right,top:top};},updatePosition:function(){if(this.el){var pos=this.getPosition();if(this.field.getHierarchyState().rtl){this.el.alignTo(this.field.inputEl,"l-l?",[pos.right+2,0],false);}
else{this.el.alignTo(this.field.inputEl,"r-r?",[-pos.right-2,0],false);}}},updateButton:function(){this.updatePosition();this.updateVisibility();},updateVisibility:function(){var el=this.el;if(el){if(this.field.readOnly||(this.hideIfEmpty&&Ext.isEmpty(this.field.getValue()))||(this.hideOnMouseOut&&!this.mouseOver)){this.el.hide();}
else{this.el.show();}
this.updatePosition();}}});
