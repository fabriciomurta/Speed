...
if(this.menu){this.menu=Ext.menu.Manager.get(this.menu);this.menu.tagLabel=this;}},beforeRender:function(){var me=this;me.callParent(arguments);Ext.apply(me.renderData,{label:me});Ext.apply(me.renderSelectors,{emptyEl:"span.x-taglabel-empty",ulEl:"ul",inputLi:"li.x-taglabel-input"});},afterRender:function(){var me=this;me.callParent(arguments);if(me.menu&&me.menuOnLeftClick){me.mon(me.menu,{scope:me,show:me.onMenuShow,hide:me.onMenuHide});}
me.mon(me.ulEl,{scope:me,click:me.handleEvent,dblclick:me.handleEvent,contextmenu:me.handleEvent,keydown:me.handleEvent});me.ulEl.unselectable();if(this.inputLi&&this.inputEl){this.inputLi.appendChild(this.inputEl);this.mon(this.inputEl,"focus",this.onInputElFocus,this);this.mon(this.inputEl,"blur",this.onInputElBlur,this);}
if(me.tags.length){var tags=me.tags;me.tags=[];me.add(tags);}},updateIndexes:function(){for(var i=0;i<this.tags.length;i++){this.tags[i].el.set({"data-index":i});}},getByField:function(name,value){var i;for(i=0;i<this.tags.length;i++){if(this.tags[i][name]==value){return this.tags[i];}}},getByText:function(text){return this.getByField("text",text);},getByValue:function(value){return this.getByField("value",value);},getTagObject:function(tag){if((tag.selected&&(this.selectionMode=="none"||!this.selectionMode))||(tag.selected&&this.selectionMode=="single"&&this.getSelected().length>0)){tag.selected=false;}
...
me.callParent();},moveInput:function(dir){if(this.inputLi){var el=dir==-1?this.inputLi.prev("li"):this.inputLi.next("li");if(el){dir==-1?this.inputLi.insertBefore(el):this.inputLi.insertAfter(el);}}},getInputPostion:function(){var el,index=this.tags.length-1;if(this.inputLi){el=this.inputLi.prev("li");if(!el){return 0;}
...
buf=['<div class="x-taglabel">','<ul class="x-taglabel-list">',body,'</ul>','</div>'];return buf.join("");};Ext.define('Ext.grid.column.Tag',{extend:'Ext.grid.column.Column',alias:['widget.tagcolumn'],defaultRenderer:function(value){return Ext.util.Format.tags(value,this.tagLabelCfg);}});Ext.define('Ext.net.TagField',{extend:'Ext.form.field.ComboBox',alias:'widget.tagfield',createNewOnBlur:false,createNewOnEnter:true,createNewOnSelect:true,collapseOnSelect:null,inputMoving:true,hideSelected:false,defaultClosable:true,initComponent:function(){var me=this;me.grow=false;this.addCls("x-tagcombo");this.multiSelect=false;this.store=this.initTagsStore();me.callParent(arguments);var task=new Ext.util.TaskRunner(50);task.start({interval:0,run:this.checkInputElChange,scope:this});this.pollTask=task;},initTagsStore:function(){if(Ext.isArray(this.store)){this.displayField="text";this.valueField="value";return new Ext.data.Store({data:this.store,fields:["text","value","selected","iconCls","disabled","cls","qTitle","qTip","width","style","closable","overCls"],autoDestroy:true});}
return store;},initEvents:function(){var me=this,events=me.checkChangeEvents,event,e,eLen=events.length;me.callParent(arguments);if(!me.enableKeyEvents){me.mon(me.inputEl,'keydown',me.onKeyDown,me);}
...
me.mun(me.inputEl,Ext.EventManager.getKeyEvent(),me.fireKey,me);for(e=0;e<eLen;e++){event=events[e];me.mun(me.inputEl,event,me.onChangeEvent);}
me.usesPropertychange=false;},setEditable:function(editable){me.inputEl.setDisplayed(editable);this.callParent();},beforeLayout:function(){var me=this;me.callParent(arguments);if(!me.tagLabel){me.initTagLabel();}},initTagLabel:function(){var me=this;(this.inputCell||this.bodyEl).addCls(["x-form-field","x-form-text","x-field-buttons-body"]);this.bodyEl.addCls("x-field-toolbar-body");this.inputEl.addCls=Ext.Function.createSequence(this.inputEl.addCls,function(cls){(this.inputCell||this.bodyEl).addCls(cls);},this);this.inputEl.removeCls=Ext.Function.createSequence(this.inputEl.removeCls,function(cls){(this.inputCell||this.bodyEl).removeCls(cls);},this);me.tagLabel=new Ext.net.TagLabel(Ext.apply({emptyText:me.emptyText||'',field:me,inputEl:me.inputEl,defaultClosable:me.defaultClosable,afterChange:Ext.Function.bind(me.afterTagsChange,me),renderTo:me.inputEl.parent().dom},{tags:this.value||[]},me.tagLabelCfg||{}));if(me.tagLabel.stacked){this.addCls("x-tagcombo-stacked");}
this.checkInputElChange();this.afterTagsChange();},applyEmptyText:Ext.emptyFn,afterTagsChange:function(){if(this.tagLabel){this.value=this.tagLabel.tags;}
...
this.callParent();},onItemClick:Ext.emptyFn,onListSelectionChange:Ext.emptyFn,getValue:function(){var state=[],value,record;Ext.each(this.value,function(obj){state.push(obj.value);},this);return state.join(this.getDelimeter());},getDelimeter:function(){if(this.tagLabel){return this.tagLabel.tagsDelimeter;}
if(this.tagLabelCfg&&this.tagLabelCfg.tagsDelimeter){return this.tagLabelCfg.tagsDelimeter;}
return",";},convertToTag:function(tag){var me=this,record=tag,obj,v;if(!record||!record.isModel){if(Ext.isObject(record)){if(Ext.isDefined(record.value)){v=record.value;}
...
else{if(!me.forceSelection){if(Ext.isObject(tag)){obj=Ext.apply({},tag);obj.text=obj[me.displayField];obj.value=obj[me.valueField];}
...
return value;},setValue:function(value,opts){var me=this,i,obj,v,matchedTags=[],len;opts=opts||{set:true};value=this.convertValue(value);if(this.store.loading){me.value=value;return me;}
...
else if(opts.remove){for(i=0;i<matchedTags.length;i++){obj=this.tagLabel.getByValue(matchedTags[i].value);if(obj){this.tagLabel.remove(obj);}}}
else{this.tagLabel.set(matchedTags);}}
else{if(opts.append){this.value=Ext.Array.push(this.value||[],matchedTags);}
...
if(this.tagLabel&&this.isExpanded){this.alignPicker();}},assertValue:function(){var me=this,value=me.inputEl?me.inputEl.dom.value:"",rec;if(me.createNewOnBlur&&value){rec=me.findRecordByDisplay(value);if(rec){this.addTagToInput(rec);}
else if(!me.forceSelection){this.addTagToInput({text:value,value:value});}}
me.inputEl.dom.value='';if(this.tagLabel){this.tagLabel.ulEl.appendChild(this.tagLabel.inputLi);}
me.collapse();},getRawValue:function(){var me=this,inputEl=me.inputEl,result;me.inputEl=false;result=me.callParent(arguments);me.inputEl=inputEl;return result;},setRawValue:function(value){var me=this,inputEl=me.inputEl,result;me.inputEl=false;result=me.callParent([value]);me.inputEl=inputEl;return result;},onKeyDown:function(e,t){var me=this,key=e.getKey(),rawValue=me.inputEl.dom.value,pos=this.getCursorPosition(),stopEvent=false;if(me.readOnly||me.disabled||!me.editable){return;}
...
if(key==e.ENTER){e.preventDefault();}
else if(key==e.LEFT){if(pos==0&&this.inputMoving){this.tagLabel.moveInput(-1,pos);this.focus();this.selectText(0,0);}}
else if(key==e.RIGHT){if(pos==rawValue.length&&this.inputMoving){this.tagLabel.moveInput(1,pos);this.focus();this.selectText(pos,pos);}}
...
if(!this.lastValue){this.lastValue=[];}},getCursorPosition:function(){var cursorPos;if(Ext.isIE){cursorPos=document.selection.createRange();cursorPos.collapse(true);cursorPos.moveStart("character",-this.inputEl.dom.value.length);cursorPos=cursorPos.text.length;}else{cursorPos=this.inputEl.dom.selectionStart;}
...
this.inputEl.focus();}},createPicker:function(){this.multiSelect=true;var picker=this.callParent(arguments);this.multiSelect=false;this.mon(picker.getSelectionModel(),{select:this.onItemSelect,deselect:this.onItemDeselect,scope:this});if(this.hideSelected){this.picker.addCls("x-hide-selection");}
return picker;},onItemSelect:function(sm,record){var me=this;if(me.ignoreSelection||!me.isExpanded){return;}
...
this.callParent(arguments);if(removeCls){picker.removeCls("x-hide-visibility");}},onExpand:function(){var me=this,keyNav=me.listKeyNav;me.callParent(arguments);if(keyNav||!me.hideSelected){return;}
...
do{index=index+direction;item=boundList.all.item(index);}while((index>0)&&(index<len)&&item.hasCls('x-boundlist-selected'));if(item.hasCls('x-boundlist-selected')){return;}}
if(item){item=item.dom;boundList.highlightItem(item);boundList.getTargetEl().scrollChildIntoView(item,false);}};}});
